import{a as r,s as n}from"./auth-cuwOp2C2.js";const d=`
...
`;async function h(){const i=document.getElementById("site-header");i&&(i.innerHTML=d,await o())}async function o(){var l;const i=document.getElementById("header-account");if(!i)return;const{data:{user:e}}=await r.auth.getUser();if(e){const{data:a}=await r.from("profiles").select("username, avatar_url, role").eq("id",e.id).maybeSingle();if(a){const s=a.avatar_url?`<img src="${a.avatar_url}" alt="${a.username}" class="header-avatar-img">`:`<span class="header-avatar-initial">${a.username[0].toUpperCase()}</span>`;i.innerHTML=`
        <div class="header-user-menu">
          <a href="/profile.html" class="header-user-link">
            <div class="header-avatar-circle">${s}</div>
            <span class="header-username-label">${a.username}</span>
          </a>
          <div class="header-user-dropdown">
            <div class="header-user-dropdown-inner">
              ...
              ${a.role==="admin"?'<a href="/admin.html"><i class="fas fa-crown"></i> Admin</a>':""}
              <button class="header-signout-btn" id="header-signout"><i class="fas fa-sign-out-alt"></i> Déconnexion</button>
            </div>
          </div>
        </div>
      `,(l=document.getElementById("header-signout"))==null||l.addEventListener("click",n)}}else i.innerHTML=`
